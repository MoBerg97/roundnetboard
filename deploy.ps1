...
Write-Host "`ncheckmark Version check passed: $pubspecVersion" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "x Flutter clean failed!" -ForegroundColor Red
    exit 1
}
...
flutter pub get
if ($LASTEXITCODE -ne 0) {
    Write-Host "x Flutter pub get failed!" -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "x Flutter build failed!" -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "x Firebase deployment failed!" -ForegroundColor Red
    exit 1
}

Write-Host "`ncheckmark Deployment complete! Version $pubspecVersion is now live." -ForegroundColor Green
